...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Persist the icon's menu-bar position across launches. Without an
        // autosaveName, macOS re-inserts the item at its default (leftmost
        // third-party) slot every launch; with it, a one-time ⌘-drag by the
        // user is remembered. Stable string = stable slot.
        item.autosaveName = "CloudToGroundAIStatusItem"
        ...
        // Periodic skill-update check, respecting the user's channel
        // preference (disabled by default privacy posture is preserved).
        // checkIfDue is a no-op when the channel is disabled or when the
        // last check was <24 h ago.
        Task {
            await SkillUpdateManager.shared.checkIfDue(prefs: Preferences.shared)
        }
    }
    ...
    @objc func openSetupWizard() {
        SetupWizardWindowController.shared.showWindow()
    }
